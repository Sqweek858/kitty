...
zstyle ':fzf-tab:*' fzf-flags '--height=40% --layout=reverse-list --border=rounded --info=inline --prompt="❯ " --pointer="▶" --marker="✓"'
...
zstyle ':fzf-tab:*' popup-mode 'never'
...
cyber_draw_parallax_header() {
  ...
  # Enhanced starfield with better randomness and depth effect
  local seed=$(( (${PARALLAX_BG_OFFSET:-0} + ${EPOCHSECONDS:-0} + ${RANDOM:-0}) % 2147483647 ))
  local i pos out=""
  ...
  PARALLAX_BG_OFFSET=$(( (PARALLAX_BG_OFFSET + 1) % 1000 ))
...
# NOTE: These hooks are handled by the preexec() and precmd() functions at the end of this file
# They are called conditionally to avoid output interference
...
parallax_start() {
  ...
  PARALLAX_BG_OFFSET=0
  STARS_DRAWN=0
  
  # Draw initial starfield
  _draw_static_stars
  
  # Note: We don't start a background process anymore to avoid output interference
  # Stars are redrawn on each precmd (after command completion)
}
...
parallax_redraw_now() {
  [[ -t 1 ]] || return
  (( PARALLAX_ENABLED )) && _draw_static_stars
}
...
preexec() {
    ...
    # Temporarily disable mouse tracking
    typeset -f cyber_disable_mouse >/dev/null 2>&1 && cyber_disable_mouse
    
    # Hide ONLY top utility bars (don't clear command output!)
    ...
    # Reset scroll region for full-screen apps (vim, less, etc.)
...
precmd() {
    ...
    [[ -n "$CYBER_LAST_COMMAND" ]] && {
        ...
        # Afișăm durata doar pentru comenzi lungi (>10s)
        [[ $dur -gt 10 ]] && printf "%b⏱  Durată: %ds%b\n" "${CYBER_COLORS[cyan]}" "$dur" "${CYBER_STYLE[reset]}"
        
        # Predicție comenzi viitoare (doar în caz de eroare)
    ...
    # Reset state
    CYBER_LAST_COMMAND=""
    CYBER_LAST_COMMAND_TIME=0
    
    # ═══════════════════════════════════════════════════════════════════════════
    # IMPORTANT: Reafișăm UI-ul PERSISTENT după comandă
    # Bara de utilități și parallax header trebuie să rămână vizibile între comenzi
    # ═══════════════════════════════════════════════════════════════════════════
    typeset -f cyber_topbar_apply_scroll_region >/dev/null 2>&1 && cyber_topbar_apply_scroll_region
    typeset -f cyber_draw_parallax_header >/dev/null 2>&1 && cyber_draw_parallax_header
    
    # Desenăm bara de utilități (dacă este activată)
    if (( CYBER_TOPBAR_ENABLED )); then
        typeset -f cyber_draw_utility_bar >/dev/null 2>&1 && cyber_draw_utility_bar 2>/dev/null
    fi
    
    # Reactivăm mouse tracking (dacă este activat)
    (( CYBER_MOUSE_UI )) && typeset -f cyber_enable_mouse >/dev/null 2>&1 && cyber_enable_mouse
}

# NOTE: Nu mai folosim add-zsh-hook separat; precmd/preexec definite mai sus se ocupă de tot
...
# Call cursor update on every key press for real-time color changes
zle-line-pre-redraw() { update_cursor_state 2>/dev/null; }
zle -N zle-line-pre-redraw

# ═══════════════════════════════════════════════════════════════════════════════
# GRADIENT BORDERED TEXT OUTPUT SYSTEM
# ═══════════════════════════════════════════════════════════════════════════════

# Generate gradient color for position
_gradient_color_at() {
  local pos=$1 total=$2 r1=$3 g1=$4 b1=$5 r2=$6 g2=$7 b2=$8
  local ratio=$(( (pos * 100) / total ))
...
# Draw gradient border box
draw_gradient_border() {
  local text="$1"
  local style="${2:-cyber}"  # cyber, rainbow, fire, ice, matrix
  ...
  for line in "${lines[@]}"; do
    local clean_line="${line//$~$'\e'\\[[0-9;]##[[:alpha:]]/}"
  ...
  case "$style" in
    cyber)
      local r1=0 g1=255 b1=255  # Cyan
      local r2=255 g2=0 b2=255  # Magenta
      ;;
    rainbow)
      local r1=255 g1=0 b1=0    # Red
      local r2=148 g2=0 b2=211  # Purple
      ;;
    fire)
      local r1=255 g1=69 b1=0   # Red-orange
      local r2=255 g2=215 b2=0  # Gold
      ;;
    ice)
      local r1=0 g1=191 b1=255  # Light blue
      local r2=240 g2=248 b2=255 # Almost white
      ;;
    matrix)
      local r1=0 g1=255 b1=0    # Green
      local r2=0 g2=100 b2=0    # Dark green
      ;;
  esac
  
  # Top border
  printf "$(_gradient_color_at 0 $width $r1 $g1 $b1 $r2 $g2 $b2)╔"
  for ((i=1; i<width-1; i++)); do
    printf "$(_gradient_color_at $i $width $r1 $g1 $b1 $r2 $g2 $b2)═"
  done
  printf "$(_gradient_color_at $((width-1)) $width $r1 $g1 $b1 $r2 $g2 $b2)╗${CYBER_STYLE[reset]}\n"
  
  # Content lines
  local line_num=0
  for line in "${lines[@]}"; do
    ((line_num++))
    local padding=$((max_len - ${#${line//$~$'\e'\\[[0-9;]##[[:alpha:]]/}}))
    local left_color=$(_gradient_color_at $((line_num * width / (${#lines[@]} + 2))) $width $r1 $g1 $b1 $r2 $g2 $b2)
    local right_color=$(_gradient_color_at $(((line_num + 1) * width / (${#lines[@]} + 2))) $width $r1 $g1 $b1 $r2 $g2 $b2)
    printf "${left_color}║${CYBER_STYLE[reset]} %s%*s ${right_color}║${CYBER_STYLE[reset]}\n" "$line" $padding ""
  done
  
  # Bottom border
  printf "$(_gradient_color_at 0 $width $r2 $g2 $b2 $r1 $g1 $b1)╚"
  for ((i=1; i<width-1; i++)); do
    printf "$(_gradient_color_at $i $width $r2 $g2 $b2 $r1 $g1 $b1)═"
  done
  printf "$(_gradient_color_at $((width-1)) $width $r2 $g2 $b2 $r1 $g1 $b1)╝${CYBER_STYLE[reset]}\n"
}

# Wrapper functions for common use cases
cyber_echo() {
  draw_gradient_border "$*" "cyber"
}

rainbow_echo() {
  draw_gradient_border "$*" "rainbow"
}

fire_echo() {
  draw_gradient_border "$*" "fire"
}

ice_echo() {
  draw_gradient_border "$*" "ice"
}

matrix_echo() {
  draw_gradient_border "$*" "matrix"
}

# Command output wrapper with border
run_with_border() {
  local style="${1:-cyber}"
  shift
  local output=$("$@" 2>&1)
  draw_gradient_border "$output" "$style"
}

# Success/Error/Warning/Info messages with borders
show_success_bordered() {
  local msg="$1"
  draw_gradient_border "✓ SUCCESS\n\n$msg" "matrix"
}

show_error_bordered() {
  local msg="$1"
  draw_gradient_border "✗ ERROR\n\n$msg" "fire"
}

show_warning_bordered() {
  local msg="$1"
  draw_gradient_border "⚠ WARNING\n\n$msg" "fire"
}

show_info_bordered() {
  local msg="$1"
  draw_gradient_border "ℹ INFO\n\n$msg" "ice"
}

# Auto-border command output (override existing functions)
show_success() {
  local msg="$1"
  if [[ -n "$msg" ]]; then
    show_success_bordered "$msg"
  fi
}
...
show_error() {
  local msg="$1"
  if [[ -n "$msg" ]]; then
    show_error_bordered "$msg"
  fi
}

# Command wrapper that adds borders to output
alias cb='run_with_border cyber'
alias rb='run_with_border rainbow'
alias fb='run_with_border fire'
alias ib='run_with_border ice'
alias mb='run_with_border matrix'
...
typeset -g PARALLAX_LAYER_COUNT=3
typeset -ga PARALLAX_LAYER_SPEEDS=(0.5 1.0 1.5)
typeset -ga PARALLAX_LAYER_DENSITIES=(5 8 12)
typeset -ga PARALLAX_LAYER_CHARS=('.' '·' '•' '✦' '❊' '✺')

# Enhanced parallax full-screen background
cyber_draw_parallax_fullscreen() {
  [[ -t 1 ]] || return
  (( PARALLAX_ENABLED )) || return
  
  local cols=${COLUMNS:-80}
  local rows=${LINES:-24}
  (( cols < 20 || rows < 10 )) && return
  
  # Save cursor position
  printf '\e[s'
  
  # Generate seed with more entropy
  local seed=$(( (${PARALLAX_BG_OFFSET:-0} * 1664525 + ${EPOCHSECONDS:-0} * 1013904223 + ${RANDOM:-0}) % 2147483647 ))
  
  # Draw each layer
  for layer in {0..2}; do
    local speed=${PARALLAX_LAYER_SPEEDS[$((layer+1))]}
    local density=${PARALLAX_LAYER_DENSITIES[$((layer+1))]}
    local offset=$(( int(PARALLAX_BG_OFFSET * speed) % cols ))
    
    # Calculate stars for this layer
    local stars=$(( (cols * rows) / density ))
    
    # Layer-specific color (depth effect)
    local brightness
    case $layer in
      0) brightness=60 ;;   # Far layer - dim
      1) brightness=120 ;;  # Mid layer - medium
      2) brightness=180 ;;  # Near layer - bright
    esac
    
    typeset -A drawn_positions
    
    for ((i=0; i<stars; i++)); do
      # LCG random number generator
      seed=$(( (seed * 1664525 + 1013904223) & 0x7fffffff ))
      local x=$(( ((seed % cols) + offset) % cols + 1 ))
      ...
      local pos_key="${x}_${y}"
      [[ -n "${drawn_positions[$pos_key]}" ]] && continue
      drawn_positions[$pos_key]=1
      
      # Select character based on layer and randomness
      seed=$(( (seed * 1664525 + 1013904223) & 0x7fffffff ))
      local char_idx=$(( (seed % ${#PARALLAX_LAYER_CHARS[@]}) + 1 ))
      local char="${PARALLAX_LAYER_CHARS[$char_idx]}"
      
      # Add color with depth
      local color_var=$(( seed % 30 ))
      local r=$((brightness + color_var - 15))
      local g=$((brightness + color_var - 10))
      local b=$((brightness + color_var + 5))
  ...
  PARALLAX_BG_OFFSET=$(( (PARALLAX_BG_OFFSET + 1) % 10000 ))
}

# Parallax layer 0 - Background stars (always visible, never interferes)
cyber_draw_parallax_layer0() {
  [[ -t 1 ]] || return
  (( PARALLAX_ENABLED )) || return
  
  # This draws on the absolute background without clearing anything
  # Uses the alternate screen buffer trick
  local cols=${COLUMNS:-80}
  local rows=${LINES:-24}
  
  # Very sparse, dim background layer
  local seed=$(( ${PARALLAX_BG_OFFSET:-0} + ${EPOCHSECONDS:-0} ))
  local stars=$(( cols * rows / 50 ))
  ...
  for ((i=0; i<stars; i++)); do
    ...
    # Very dim stars
    local brightness=$(( 40 + (seed % 20) ))
    
    # Save cursor, draw, restore
    printf '\e[s\e[%d;%dH\e[38;2;%d;%d;%dm·\e[0m\e[u' "$y" "$x" "$brightness" "$brightness" "$((brightness + 10))"
  done
}
